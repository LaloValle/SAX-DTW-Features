...
def add_warping(time_series,amount_warping:float=0.20,convert_int:bool=False,upper_bound_alphabet:int=7):
    """
    ...
    if convert_int: 
        ...
        if (-0 in list_unique) or ((upper_bound_alphabet+1) in list_unique):
            # Hard coded the upper limit of the alphabet to 7 simbols
            warped_time_series[warped_time_series <= 0] = 0
            warped_time_series[warped_time_series > upper_bound_alphabet] = upper_bound_alphabet
...
def minimum_warping_window(time_series_set,prediction_function,alphabet_size:int=0,number_iterations:int=10,upper_bound_window:int=0,lower_bound_window:int=2,representatives:dict=dict(),convert_int:bool=False,verbose:bool=False,k=10):
    ...
    table_WindowSize_vs_Accuracy = pd.DataFrame(WindowSize_vs_Accuracy,columns=range(2,upper_bound_window+1),index=[f'Iteration {i}' for i in range(1,number_iterations+1)]+['Global Avg.'])
    # Saves the array
