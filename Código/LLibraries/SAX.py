...
def PAA(time_series,words:int):
    time_series_length = len(time_series)
    #
    #   Base cases
    #
    # Same number of words as the elements in the series
    if time_series_length == words: return time_series
    # One interval equals to the mean of the whole series
    if words == 1: return np.mean(time_series)
    # Verifies the number of words is smaller than the lenght of the time series
    if time_series_length < words: return False

    # When the number of words is a multiple of the time series length
    # a simple equally split of the time series is perfomed
    if (time_series_length % words) == 0:
        # The new approximate time series gets splited into equally distributed words of rows
        aproximate_time_series = time_series.reshape((words,-1))
        return np.mean(aproximate_time_series,axis=1)
    # When the number of words cannot be achieved whit an equally-sized partition of the time series
    # the frame must be resized to obtain an equi-sized
    if time_series_length % words:
        #==============
        # Second method
        #==============
        #
        ...
        # ******* Best option ********
        #
        # Array with the indexes that will be used for the compute of the mean of the number of words specified
        indexes = ((np.arange(0, time_series_length*words)) // words).reshape((words,-1))
